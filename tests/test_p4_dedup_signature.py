...
class P4DedupSignatureTests(unittest.TestCase):
    ...
    def test_coverage_signature_distinguishes_same_reasons_by_member_calls(
        self,
    ) -> None:
        file_hits_a = [
            (
                5,
                TestFileIndex(
                    relative_path="scroll_list03.ets", member_calls={"scrollToIndex"}
                ),
                ["calls Button()", "mentions button"],
            )
        ]
        file_hits_b = [
            (
                5,
                TestFileIndex(
                    relative_path="layout_column.ets", member_calls={"justifyContent"}
                ),
                ["calls Button()", "mentions button"],
            )
        ]
        ...
        self.assertNotEqual(
            coverage_signature(file_hits_a), coverage_signature(file_hits_b)
        )

    def test_deduplicate_keeps_projects_with_distinct_member_call_signatures(
        self,
    ) -> None:
        sig_scroll = coverage_signature(
            [
                (
                    5,
                    TestFileIndex(
                        relative_path="scroll_list03.ets",
                        member_calls={"scrollToIndex"},
                    ),
                    ["calls Button()", "mentions button"],
                )
            ]
        ...
        sig_layout = coverage_signature(
            [
                (
                    5,
                    TestFileIndex(
                        relative_path="layout_column.ets",
                        member_calls={"justifyContent"},
                    ),
                    ["calls Button()", "mentions button"],
                )
            ]
        ...
        self.assertEqual(
            [item["project"] for item in result], ["scroll_list03", "layout_column"]
        )
