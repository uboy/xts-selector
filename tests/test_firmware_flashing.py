...
class FlashingTests(unittest.TestCase):
    ...
    def test_list_rockchip_devices_parses_loader_output(self) -> None:
        ...
        with mock.patch(
            "arkui_xts_selector.flashing._run_command", return_value=completed
        ):
    ...
    def test_list_rockchip_devices_treats_failed_libusb_init_as_no_device(self) -> None:
        completed = SimpleNamespace(
            returncode=255, stdout="failed to init libusb!\n", stderr=""
        )
        with mock.patch(
            "arkui_xts_selector.flashing._run_command", return_value=completed
        ):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            def fake_run(
                ...
                if command == [str(flash_tool), "LD"]:
                    if (
                        sum(1 for item in commands if item == [str(flash_tool), "LD"])
                        == 1
                    ):
            ...
            def fake_stream(
                ...
                self.assertEqual(
                    command,
                    [sys.executable, str(flash_py), "-a", "-i", str(image_root)],
                )
                if progress_callback is not None:
                    progress_callback("Write gpt ok.")
                    progress_callback("write progress 35%")
                return SimpleNamespace(
                    returncode=0, stdout="Reset Device OK.\n", stderr=""
                )

            with (
                mock.patch(
                    "arkui_xts_selector.flashing.resolve_flash_py_path",
                    return_value=flash_py,
                ),
                mock.patch(
                    "arkui_xts_selector.flashing.resolve_hdc_path", return_value=hdc
                ),
                mock.patch(
                    "arkui_xts_selector.flashing.infer_flash_tool_path",
                    return_value=flash_tool,
                ),
                mock.patch(
                    "arkui_xts_selector.flashing._run_streaming_command",
                ...
                mock.patch(
                    "arkui_xts_selector.flashing._run_command", side_effect=fake_run
                ),
            ...
            self.assertIn(
                [str(hdc), "-t", "SER1", "target", "boot", "-bootloader"], commands
            )
            self.assertTrue(hdc_envs)
            self.assertEqual(
                hdc_envs[0]["LD_LIBRARY_PATH"].split(":")[0], str(toolchains.resolve())
            )
