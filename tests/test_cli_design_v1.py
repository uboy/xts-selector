...
class CliDesignV1Tests(unittest.TestCase):
    ...
    def test_parse_pr_number_supports_pull_and_pulls_urls(self) -> None:
        ...
        self.assertEqual(
            parse_pr_number(
                "https://gitcode.com/openharmony/arkui_ace_engine/pulls/83145"
            ),
            "83145",
        )
        self.assertEqual(
    ...
    def test_parse_owner_repo_helpers_support_pulls_and_remote_urls(self) -> None:
        ...
        self.assertEqual(
            parse_owner_repo_from_remote_url(
                "git@gitcode.com:openharmony/arkui_ace_engine.git"
            ),
            ("openharmony", "arkui_ace_engine"),
        )
        self.assertEqual(
            parse_owner_repo_from_remote_url(
                "https://codehub.example.com/group/project.git"
            ),
            ("group", "project"),
        )
    ...
    def test_load_ini_git_host_config_supports_codehub_section(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            url, token = load_ini_git_host_config(
                str(ini_path), Path(tmpdir), "codehub"
            )

        self.assertEqual(url, "https://codehub.example.com/")
        self.assertEqual(token, "codehub-secret")

    def test_apply_ranking_rules_config_reloads_family_groups_and_rank_weight(
        self,
    ) -> None:
        ...
        try:
            ...
            self.assertEqual(
                coverage_capability_key("TabContent"), "navigation_stack.tabs"
            )
    ...
    def test_default_path_rules_include_mixed_checkboxgroup_aliases(self) -> None:
        ...
        self.assertIn(
            "CheckBoxGroupConfiguration", mapping.pattern_alias["checkboxgroup"]
        )
    ...
    def test_fetch_pr_changed_files_via_api_accepts_wrapped_files_payload(self) -> None:
            ...
            return_value=_Response(
                {
                    "files": [
                        {
                            "filename": "frameworks/core/components_ng/pattern/button/button_pattern.cpp"
                        }
                    ]
                }
            ),
        ...
        self.assertTrue(
            str(changed[0]).endswith(
                "frameworks/core/components_ng/pattern/button/button_pattern.cpp"
            )
        )

    def test_fetch_pr_changed_files_via_api_supports_codehub_changes_payload_with_path_fallback(
        self,
    ) -> None:
        class _Response:
            def __init__(self, payload: object) -> None:
        ...
        def _fake_urlopen(request: object, timeout: int = 20) -> _Response:
            ...
            if "/isource/merge_requests/" in full_url:
                raise urllib.error.HTTPError(
                    full_url, 404, "Not Found", hdrs=None, fp=None
                )
            return _Response(
                {
                    "changes": [
                        {
                            "new_path": "frameworks/core/interfaces/native/implementation/flow_item_modifier.cpp"
                        }
                    ]
                }
            )
        ...
        with mock.patch(
            "arkui_xts_selector.cli.urllib.request.urlopen", side_effect=_fake_urlopen
        ):
            changed = fetch_pr_changed_files_via_api(
                api_kind="codehub",
                api_url="https://codehub.example.com",
        ...
        self.assertIn(
            "/api/v4/projects/group%2Fproject/isource/merge_requests/12/changes",
            seen_urls[0],
        )
        self.assertIn(
            "/api/v4/projects/group%2Fproject/merge_requests/12/changes", seen_urls[1]
        )
    ...
    def test_resolve_pr_changed_files_prefers_api_when_requested(self) -> None:
        ...
        with (
            mock.patch(
                "arkui_xts_selector.cli.resolve_pr_owner_repo",
                return_value=("openharmony", "arkui_ace_engine"),
            ),
            mock.patch(
                "arkui_xts_selector.cli.fetch_pr_metadata_via_api",
                return_value={"number": 83145},
            ),
            mock.patch(
    ...
    def test_resolve_pr_changed_files_loads_codehub_credentials_from_ini(self) -> None:
        ...
        self.assertEqual(metadata_fetch.call_args.kwargs["api_kind"], "codehub")
        self.assertEqual(
            metadata_fetch.call_args.kwargs["api_url"], "https://codehub.example.com"
        )
        self.assertEqual(metadata_fetch.call_args.kwargs["token"], "secret-token")

    def test_resolve_pr_changed_files_auto_falls_back_to_git_when_api_fails(
        self,
    ) -> None:
        app_config = AppConfig(
            repo_root=Path("/repo"),
        ...
        with (
            ...
            mock.patch(
                "arkui_xts_selector.cli.fetch_pr_changed_files", return_value=expected
            ) as git_fetch,
        ):
            resolved = resolve_pr_changed_files(
                app_config,
    ...
    def test_resolve_json_output_path_defaults_to_cwd_report_file(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                os.chdir(old_cwd)
        self.assertEqual(
            resolved, (Path(tmpdir) / "arkui_xts_selector_report.json").resolve()
        )
    ...
    def test_write_json_report_writes_file_when_stdout_disabled(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            written = write_json_report(
                {"ok": True}, json_to_stdout=False, json_output_path=target
            )
            payload = target.read_text(encoding="utf-8").strip()
        self.assertEqual(written, target.resolve())
        self.assertEqual(
            payload, json.dumps({"ok": True}, ensure_ascii=False, indent=2)
        )
    ...
    def test_write_json_report_prints_stdout_when_requested(self) -> None:
        ...
        with redirect_stdout(buffer):
            written = write_json_report(
                {"ok": True}, json_to_stdout=True, json_output_path=None
            )
    ...
    def test_normalize_changed_files_prefers_existing_git_repo_relative_path(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
...
""".strip(),
    ...
    def test_infer_signals_adds_canonical_alias_symbols_for_tabcontent_and_xcomponent_paths(
        self,
    ) -> None:
    ...
    def test_infer_signals_reads_generated_ets_exported_types_and_public_methods(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
    ...
    def test_infer_signals_reads_generated_ets_imported_types_and_ohos_modules(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
    ...
    def test_infer_signals_demotes_incidental_imports_from_changed_ets_source(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
            path = (
    ...
    def test_build_query_signals_collects_method_hints_from_composite_mapping(
        self,
    ) -> None:
    ...
    def test_score_file_keeps_weak_import_signals_below_direct_evidence_threshold(
        self,
    ) -> None:
        file_index = TestFileIndex(
    ...
    def test_match_changed_file_exclusion_matches_native_unit_test_path(self) -> None:
        matched = match_changed_file_exclusion(
            Path(
                "/tmp/work/test/unittest/core/pattern/text_input/text_input_modify_test.cpp"
            ),
            Path("/tmp/work"),
        ...
        self.assertEqual(
            matched["changed_file"],
            "test/unittest/core/pattern/text_input/text_input_modify_test.cpp",
        )
        self.assertEqual(matched["rule_id"], "native_unit_tests_root")

    def test_filter_changed_files_for_xts_excludes_unit_test_and_keeps_framework_file(
        self,
    ) -> None:
        changed_files = [
            Path(
            ...
            Path(
                "/tmp/work/foundation/arkui/ace_engine/frameworks/core/components_ng/pattern/slider/slider_pattern.cpp"
            ),
        ]
    ...
    def test_filter_changed_files_for_xts_excludes_assembled_advanced_ui_wrapper(
        self,
    ) -> None:
        changed_files = [
            Path(
                "/tmp/work/foundation/arkui/ace_engine/advanced_ui_component/chipgroup/source/chipgroup.ets"
            ),
            Path(
                "/tmp/work/foundation/arkui/ace_engine/advanced_ui_component_static/assembled_advanced_ui_component/@ohos.arkui.advanced.ChipGroup.ets"
            ),
        ]
        ...
        self.assertEqual(
            excluded[0]["rule_id"], "generated_advanced_ui_assembled_wrappers"
        )

    def test_load_changed_file_exclusion_config_merges_configured_prefixes(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
                        ...
                        "rules": [
                            {
                                ...
                                "how_to_identify": [
                                    "Path starts with generated/docs/."
                                ],
                            }
                        ],
                    }
    ...
    def test_build_next_steps_uses_latest_available_daily_tag_and_date_when_config_is_empty(
        self,
    ) -> None:
        app_config = AppConfig(
        ...
        def fake_list_daily_tags(
            component: str, branch: str = "master", count: int = 1, **_: object
        ) -> list[DailyBuildInfo]:
        ...
        with mock.patch(
            "arkui_xts_selector.report_human.list_daily_tags",
            side_effect=fake_list_daily_tags,
        ):
        ...
        self.assertIn(
            "--sdk-build-tag 20260409_101500",
            step_commands["Download SDK For Selection"],
        )
        self.assertIn(
            "--sdk-date 20260409", step_commands["Download SDK For Selection"]
        )
        self.assertIn(
            "--daily-build-tag 20260408_223344", step_commands["Download tests"]
        )
        self.assertIn("--daily-date 20260408", step_commands["Download tests"])
        self.assertIn(
            "--firmware-build-tag 20260407_112233", step_commands["Download firmware"]
        )
        self.assertIn("--firmware-date 20260407", step_commands["Download firmware"])
        self.assertIn(
            "--firmware-build-tag 20260407_112233",
            step_commands["Flash daily firmware"],
        )
        self.assertIn("--firmware-date 20260407", step_commands["Flash daily firmware"])

    def test_build_next_steps_uses_explicit_tag_and_derived_date_without_network_lookup(
        self,
    ) -> None:
        app_config = AppConfig(
            repo_root=Path("/tmp/repo"),
        ...
        step_commands = {step["step"]: step["command"] for step in steps}
        self.assertIn(
            "--sdk-build-tag 20260406_204500",
            step_commands["Download SDK For Selection"],
        )
        self.assertIn(
            "--sdk-date 20260406", step_commands["Download SDK For Selection"]
        )
        self.assertIn(
            "--daily-build-tag 20260405_193000", step_commands["Download tests"]
        )
        self.assertIn("--daily-date 20260405", step_commands["Download tests"])
        self.assertIn(
            "--firmware-build-tag 20260404_081500", step_commands["Download firmware"]
        )
        self.assertIn("--firmware-date 20260404", step_commands["Download firmware"])

    def test_build_next_steps_use_ohos_wrapper_subcommands_when_configured(
        self,
    ) -> None:
        app_config = AppConfig(
            repo_root=Path("/tmp/repo"),
        ...
        step_commands = {step["step"]: step["command"] for step in steps}
        self.assertTrue(
            step_commands["Download SDK For Selection"].startswith("ohos download sdk ")
        )
        self.assertTrue(
            step_commands["Download tests"].startswith("ohos download tests ")
        )
        self.assertTrue(
            step_commands["Download firmware"].startswith("ohos download firmware ")
        )
        self.assertTrue(
            step_commands["Flash daily firmware"].startswith("ohos device flash ")
        )
    ...
    def test_build_next_steps_omits_sdk_step_for_from_report_flow(self) -> None:
        ...
        report = {
            ...
            "coverage_recommendations": {
                "required_target_keys": [],
                "recommended_target_keys": ["suite1"],
            },
    ...
    def test_build_next_steps_adds_compare_step_when_safe_base_run_exists(self) -> None:
        ...
        report = {
            ...
            "selector_run": {
                "label": "fix",
                "selector_report_path": "/tmp/report.json",
            },
        }
        ...
        self.assertEqual(
            step_commands["Compare with base run"], "ohos xts compare baseline fix"
        )
    ...
    def test_print_human_marks_targets_missing_from_current_artifacts(self) -> None:
        report = {
            ...
            "built_artifact_index": {
                "status": "built",
                "module_info_entries": ["ActsAvailable"],
                "testcase_modules_count": 1,
        ...
        report = {
                    ...
                    "run_targets": [
                        {
                            ...
                            "scope_tier": "focused",
                            "scope_reasons": [
                                "top matching files stay in target API paths: button"
                            ],
                        ...
                        },
                        {
                            ...
                            "scope_tier": "broad",
                            "scope_reasons": [
                                "project path looks broad or umbrella-like: modifier"
                            ],
            ...
            "execution_overview": {
                "selected_target_keys": [
                    "test/xts/acts/arkui/ace_ets_module_modifier_static/Test.json"
                ]
            },
        }
        buffer = io.StringIO()
    ...
    def test_print_human_run_only_mode_is_compact(self) -> None:
        report = {
                            ...
                            "execution_results": [
                                {
                                    ...
                                    "duration_s": 12.5,
                                    "returncode": 0,
                                    "case_summary": {"passed": 10, "failed": 0},
                                    "result_path": "/tmp/xdevice/result",
                                }
                            ],
            ...
            "execution_overview": {
                ...
                "executed": True,
                "run_tool": "xdevice",
                "run_priority": "recommended",
                "parallel_jobs": 1,
            },
            "execution_preflight": {
                "status": "passed",
                "plan_count": 1,
                "selected_tools": ["xdevice"],
                "connected_devices": ["SER1"],
            },
            "execution_summary": {
                "planned_run_count": 1,
                "passed": 1,
    ...
    def test_write_selected_tests_report_creates_companion_json(self) -> None:
        ...
        self.assertEqual(
            payload["requested_test_names"], ["ace_ets_module_modifier_static"]
        )
    ...
    def test_sort_project_results_prefers_scope_tier_before_raw_score(self) -> None:
        ...
        self.assertEqual(
            [item["project"] for item in ranked], ["focused/project", "broad/project"]
        )
    ...
    def test_split_scope_groups_separates_primary_from_broad(self) -> None:
        ...
        self.assertEqual(
            [item["project"] for item in primary], ["direct/project", "focused/project"]
        )
        self.assertEqual([item["project"] for item in broader], ["broad/project"])

    def test_build_global_coverage_recommendations_prefers_new_coverage_then_marks_duplicates_optional(
        self,
    ) -> None:
        ...
        self.assertEqual(
            recommendations["optional_duplicates"][0]["new_coverage_count"], 0
        )

    def test_build_source_profile_groups_navigation_related_tokens_into_single_family(
        self,
    ) -> None:
        profile = build_source_profile(
            ...
            raw_path=Path(
                "/tmp/repo/frameworks/bridge/declarative_frontend/engine/jsi/components/arkts_native_navigation.ets"
            ),
        )
    ...
    def test_infer_project_family_profile_extracts_direct_web_family(self) -> None:
        ...
        profile = infer_project_family_profile(
            ...
            [
                (
                    42,
                    TestFileIndex(
                        relative_path="entry/src/main/ets/MainAbility/pages/web/web.ets"
                    ),
        ...
        self.assertGreater(
            profile["family_representative_quality"]["web"],
            profile["family_quality"]["web"],
        )
        ...
        profile = infer_project_family_profile(
            ...
            [
                (
                    12,
                    TestFileIndex(
                        relative_path="entry/src/main/ets/MainAbility/pages/conponentadd/web.ets"
                    ),
    ...
    def test_suite_source_family_gains_rewards_direct_overlap_and_penalizes_umbrella(
        self,
    ) -> None:
    ...
    def test_suite_source_family_representative_scores_reward_source_token_overlap(
        self,
    ) -> None:
        ...
        self.assertGreater(
            narrow_scores["navigation_stack"], broad_scores["navigation_stack"]
        )
    ...
    def test_build_global_coverage_recommendations_prefers_direct_family_owner_over_umbrella_candidate(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
        ...
        self.assertEqual(
            recommendations["recommended_target_keys"], ["proj/global_web/Test.json"]
        )

    def test_build_global_coverage_recommendations_prefers_exact_source_token_owner_inside_family(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
        ...
        self.assertEqual(
            recommendations["recommended_target_keys"], ["proj/navigation4/Test.json"]
        )

    def test_build_global_coverage_recommendations_prefers_focus_owner_for_fallback_only_source(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
        ...
        self.assertEqual(
            recommendations["required_target_keys"], ["proj/advance_chip/Test.json"]
        )

    def test_build_global_coverage_recommendations_prefers_exact_capability_owner_inside_family(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
        ...
        self.assertEqual(
            recommendations["recommended_target_keys"], ["proj/tabs/Test.json"]
        )

    def test_build_global_coverage_recommendations_breaks_representative_ties_by_umbrella_penalty(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
    ...
    def test_build_global_coverage_recommendations_collapses_same_family_changed_files(
        self,
    ) -> None:
        recommendations = build_global_coverage_recommendations(
            [
                {
                    "source": {
                        "key": "changed_file:navigation.ets",
                        "type": "changed_file",
                        "value": "navigation.ets",
                    },
                    "source_profile": {
                        "key": "changed_file:navigation.ets",
                        "type": "changed_file",
        ...
        self.assertEqual(
            recommendations["recommended"][0]["new_families"], ["navigation_stack"]
        )
    ...
    def test_classify_project_scope_marks_generic_common_attrs_suite_as_broad(
        self,
    ) -> None:
        project = TestProjectIndex(
        ...
        file_hits = [
            (
                24,
                TestFileIndex(
                    relative_path="entry/src/main/ets/MainAbility/pages/focusControl/FocusControl.ets"
                ),
            ...
            (
                10,
                TestFileIndex(
                    relative_path="entry/src/main/ets/MainAbility/pages/index/index.ets"
                ),
                ["mentions button"],
            ),
        ]
    ...
    def test_classify_project_scope_marks_button_specific_suite_as_primary(
        self,
    ) -> None:
        project = TestProjectIndex(
        ...
        file_hits = [
            (
                32,
                TestFileIndex(
                    relative_path="entry/src/main/ets/MainAbility/pages/button/ButtonApi.ets"
                ),
    ...
    def test_build_xdevice_command_uses_ready_report_path_without_placeholder(
        self,
    ) -> None:
    ...
    def test_print_human_shows_coverage_recommendations_and_optional_duplicates(
        self,
    ) -> None:
        optional_duplicates = []
    ...
    def test_build_coverage_run_commands_preserve_runtime_state_settings(self) -> None:
        ...
        self.assertIn(
            "--runtime-state-root /tmp/custom_runtime_state", required_command
        )
    ...
    def test_build_coverage_run_commands_use_ohos_wrapper_when_configured(self) -> None:
        ...
        self.assertTrue(
            commands[0]["command"].startswith(
                "ohos xts run --from-report /tmp/report.json "
            )
        )
    ...
    def test_print_human_shows_zero_coverage_recommendations_for_multi_source_reports(
        self,
    ) -> None:
        report = {
            "repo_root": "/tmp/repo",
    ...
    def test_classify_project_variant_from_static_hap(self) -> None:
        variant = classify_project_variant(
            "ace_ets_component/foo", ["ActsFooStaticTest.hap"]
        )
    ...
    def test_filter_project_results_by_relevance_modes(self) -> None:
        ...
        balanced, balanced_summary = filter_project_results_by_relevance(
            ranked, "balanced"
        )
    ...
    def test_candidate_bucket_requires_non_lexical_evidence_for_must_run(self) -> None:
        ...
        self.assertEqual(
            candidate_bucket(
                30, True, evidence_profile={"direct_type_hint_keys": ["Button"]}
            ),
            "must-run",
        )
    ...
    def test_format_report_filters_symbol_query_projects_by_variant(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            dynamic_test_json.write_text(
                json.dumps(
                    {
                        "driver": {"module-name": "entry"},
                        "kits": [{"test-file-name": ["ActsButtonDynamicTest.hap"]}],
                    }
                ),
                encoding="utf-8",
            )
        ...
        self.assertEqual(
            projects_out[0]["project"], "test/xts/acts/arkui/button_static"
        )
        ...
        self.assertEqual(
            report["symbol_queries"][0]["effective_variants_mode"], "static"
        )
    ...
    def test_format_report_debug_trace_adds_unresolved_diagnostics(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            test_json.write_text(
                json.dumps(
                    {
                        "driver": {"module-name": "entry"},
                        "kits": [{"test-file-name": ["ActsCommonAttrs.hap"]}],
                    }
                ),
                encoding="utf-8",
            )
        ...
        self.assertEqual(
            result["unresolved_debug"]["reason"], result["unresolved_reason"]
        )
    ...
    def test_resolve_variants_mode_auto_uses_semantics_for_native_implementation_file(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
            source = (
                Path(tmpdir)
                / "frameworks/core/interfaces/native/implementation/helper.cpp"
            )
...
""".strip(),
    ...
    def test_build_unresolved_analysis_exposes_reasoning_fields(self) -> None:
        analysis = build_unresolved_analysis(
            ...
            [
                {
                    "score": 42,
                    "project": "test/xts/acts/arkui/ace_ets_component_common_seven_attrs_align_static",
                },
                {
                    "score": 38,
                    "project": "test/xts/acts/arkui/ace_ets_component_common_seven_attrs_border_static",
                },
                {
                    "score": 34,
                    "project": "test/xts/acts/arkui/ace_ets_component_common_seven_attrs_translate_static",
                },
            ],
        )
    ...
    def test_unresolved_reason_skips_content_modifier_warning_without_signal(
        self,
    ) -> None:
        reason = unresolved_reason(
            ...
            [
                {
                    "score": 24,
                    "project": "ohos_master/test/xts/acts/arkui/ace_ets_component_seven/ace_ets_component_common_seven_attrs_align_static",
                },
                {
                    "score": 22,
                    "project": "ohos_master/test/xts/acts/arkui/ace_ets_component_seven/ace_ets_component_common_seven_attrs_overlay_static",
                },
                {
                    "score": 20,
                    "project": "ohos_master/test/xts/acts/arkui/ace_ets_component_seven/ace_ets_component_common_seven_attrs_borderImage_static",
                },
            ],
        )
    ...
    def test_resolve_variants_mode_auto_keeps_both_for_pattern_backend_files(
        self,
    ) -> None:
    ...
    def test_diversify_symbol_query_projects_injects_dynamic_exclusive_candidate(
        self,
    ) -> None:
        project_results = [
            {
    ...
    def test_diversify_symbol_query_projects_injects_both_exclusive_surfaces(
        self,
    ) -> None:
        project_results = [
            {
    ...
    def test_restrict_explicit_surface_projects_prefers_exclusive_surface(self) -> None:
        ...
        shown = restrict_explicit_surface_projects(
            project_results, "dynamic", explicit_surface_query=True
        )

        self.assertEqual([item["project"] for item in shown], ["dynamic-1"])

    def test_restrict_explicit_surface_projects_falls_back_to_supporting_projects(
        self,
    ) -> None:
        project_results = [
            {
    ...
    def test_identifier_call_scores_less_than_import_and_less_than_combined(
        self,
    ) -> None:
    ...
    def test_convergence_bonus_increases_score_for_multiple_files(self) -> None:
        ...
        fi2 = TestFileIndex(
            relative_path="test/button/B.ets", imported_symbols={"Button"}
        )
        fi3 = TestFileIndex(
            relative_path="test/button/C.ets", imported_symbols={"Button"}
        )
        fi4 = TestFileIndex(
            relative_path="test/button/D.ets", imported_symbols={"Button"}
        )
        ...
        self.assertTrue(
            any("convergence" in r for r in reasons_four),
            "convergence bonus must appear in reasons",
        )
        self.assertFalse(
            any("convergence" in r for r in reasons_one),
    ...
    def test_deduplicate_disabled_when_zero(self) -> None:
        ...
        projects = [
            {
                "_coverage_sig": frozenset(["calls Button()"]),
                "score": 8,
                "project": "a",
            },
            {
    ...
    def test_deduplicate_keeps_n_per_signature(self) -> None:
        ...
        projects = [
            ...
            {
                "_coverage_sig": sig_explicit,
                "score": 22,
                "project": "button_modifier_a",
            },
            {
                "_coverage_sig": sig_explicit,
                "score": 21,
                "project": "button_modifier_b",
            },
        ]
    ...
    def test_deduplicate_unique_signatures_all_kept(self) -> None:
        ...
        projects = [
            {
                "_coverage_sig": frozenset(["member call .borderColor()"]),
                "score": 22,
                "project": "a",
            },
            {
                "_coverage_sig": frozenset(["member call .alignContent()"]),
                "score": 22,
                "project": "b",
            },
            {
                "_coverage_sig": frozenset(["member call .fontWeight()"]),
                "score": 22,
                "project": "c",
            },
        ]
        ...
        self.assertEqual(
            len(result), 3, "All unique signatures must be kept even with keep=1"
        )
    ...
    def test_candidate_bucket_boundaries(self) -> None:
        ...
        self.assertEqual(
            candidate_bucket(
                24, True, evidence_profile={"direct_type_hint_keys": ["Button"]}
            ),
            "must-run",
        )
        self.assertEqual(
            candidate_bucket(
                24,
                True,
                evidence_profile={"direct_member_hint_keys": ["Button.onClick"]},
            ),
            "must-run",
        )
        self.assertEqual(
            candidate_bucket(
                100, True, evidence_profile={"direct_type_hint_keys": ["Button"]}
            ),
            "must-run",
        )
...
class SubstringMatchingFixTests(unittest.TestCase):
    ...
    def test_short_query_does_not_trigger_composite(self) -> None:
        ...
        signals = build_query_signals(
            "content", sdk_index, content_index, mapping_config
        )
...
class CoverageSignaturePathCategoryTests(unittest.TestCase):
    ...
    def test_signature_includes_path_category(self) -> None:
        ...
        sig1 = coverage_signature(
            hits, "ace_ets_component_common_seven_attrs_borderColor_static"
        )
        sig2 = coverage_signature(
            hits, "ace_ets_component_common_seven_attrs_backgroundColor_static"
        )
...
class CacheIsolationTests(unittest.TestCase):
    ...
    def test_load_or_build_projects_does_not_rewrite_cache_on_full_hit(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            projects_first, cache_used_first = load_or_build_projects(
                xts_root, cache_file
            )
            ...
            with mock.patch(
                ...
                side_effect=AssertionError(
                    "cache rewrite should be skipped on full hit"
                ),
            ):
        ...
        self.assertEqual(
            [project.relative_root for project in projects_second],
            [projects_first[0].relative_root],
        )
    ...
    def test_load_or_build_projects_backfills_meta_without_project_hashes(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            pages_dir = project_root / "pages"
            pages_dir.mkdir(parents=True, exist_ok=True)
            (pages_dir / "index.ets").write_text(
                "Button('demo').role(ButtonRole.Normal)\n", encoding="utf-8"
            )
            cache_file = Path(tmpdir) / "xts-cache.json"
            ...
            os.utime(
                cache_file,
                ns=(cache_stat.st_atime_ns, cache_stat.st_mtime_ns + 10_000_000),
            )
    ...
    def test_score_project_loads_lazy_cached_files_on_demand(self) -> None:
        ...
        self.assertTrue(reasons)
        self.assertEqual(
            file_hits[0][1].relative_path,
            "test/xts/acts/arkui/demo_static/pages/index.ets",
        )
        self.assertEqual(len(project.files), 1)

    def test_project_might_match_exact_api_prefilter_skips_broad_path_only_hits(
        self,
    ) -> None:
        project = TestProjectIndex.from_dict(
            {
    ...
    def test_select_candidate_projects_prefers_exact_api_shortlist_for_file_only_lineage(
        self,
    ) -> None:
        ...
        self.assertEqual(
            [project.relative_root for project in all_projects],
            [broad_project.relative_root, exact_project.relative_root],
        )
        self.assertEqual(
            [project.relative_root for project in shortlisted],
            [exact_project.relative_root],
        )

    def test_select_candidate_projects_falls_back_to_broad_shortlist_when_exact_pass_is_empty(
        self,
    ) -> None:
        broad_project = TestProjectIndex.from_dict(
            {
