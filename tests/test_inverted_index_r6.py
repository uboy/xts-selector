...
def test_consumers_for_member_name_canonical_url_encoded_keys():
    ...
    idx = InvertedIndex(by_api={
        "api:v1:arkui:event_or_method:common#ButtonAttribute%23role": [_make_consumer("buttonproj")],
        "api:v1:arkui:event_or_method:common#CheckboxAttribute%23role": [_make_consumer("checkproj")],
    })
    consumers = idx.consumers_for_member_name("role")
    assert len(consumers) == 2
    consumers_filtered = idx.consumers_for_member_name("role", parent_filter="ButtonAttribute")
