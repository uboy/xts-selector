...
class FanOutGraphStructureTests(unittest.TestCase):
    ...
    def test_provides_static_modifier_edges_exist(self) -> None:
        ...
        provides_edges = [
            ...
            for e in self.graph.edges.values()
            if (
                e.edge_type == EdgeType.PROVIDES_STATIC_MODIFIER.value
    ...
    def test_button_content_modifier_has_direct_consumer(self) -> None:
        ...
        uses_edges = [
            ...
            for e in self.graph.edges.values()
            if (
                e.edge_type == EdgeType.USES_API.value
        ...
        self.assertEqual(
            uses_edges[0].consumer_usage_confidence,
            "strong",
            "Button.contentModifier consumer should have strong confidence",
        )
        self.assertEqual(
            uses_edges[0].evidence.provenance,
            "parser",
    ...
    def test_list_content_modifier_no_direct_consumer(self) -> None:
        ...
        uses_edges = [
            ...
            for e in self.graph.edges.values()
            if (
                e.edge_type == EdgeType.USES_API.value and e.to_node == list_modifier_id
            )
        ]
        self.assertEqual(
            len(uses_edges),
...
class FanOutFalseNegativeRiskTests(unittest.TestCase):
    ...
    def test_direct_consumer_has_lower_risk(self) -> None:
        ...
        self.assertEqual(
            risk, "low", "Strong direct consumer should have low false-negative risk"
        )
