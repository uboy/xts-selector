...
def _make_batch_result(pr_number: int, *, targets: list[str] | None = None,
                       policy: str = "ok", status: str = "ok",
                       unresolved_count: int = 0) -> dict:
...
def _run_generator(batch_results: list[dict], pr_caches: dict[int, dict]) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pr_num, cache_data in pr_caches.items():
            pr_cache_path = cache_dir / "gitcode_com" / "openharmony" / "arkui_ace_engine" / f"PR_{pr_num}.json"
        ...
        cmd = [
            sys.executable, str(SCRIPTS_DIR / "generate_golden_ground_truth.py"),
            "--batch-results", batch_path,
            "--pr-cache-dir", str(cache_dir),
            "--output", output_path,
            "--target-count", str(len(batch_results)),
        ]
...
class TestTautologyRemoval:
    ...
    def test_no_approved_annotation_status(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert status != "approved", f"PR {pr['pr_number']} has annotation_status='approved', should be 'candidate'"
            assert status == "candidate", f"PR {pr['pr_number']} has unexpected annotation_status='{status}'"
    ...
    def test_no_auto_verified_label_source(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert source != "auto_verified", f"PR {pr['pr_number']} has label_source='auto_verified', should be 'helper_script'"
            assert source == "helper_script", f"PR {pr['pr_number']} has unexpected label_source='{source}'"

    def test_empty_reviewer_decision_must_run(self):
        """Script must leave reviewer_decision.must_run EMPTY."""
        batch_results = [
            _make_batch_result(100, targets=["ace_ets_test_button", "ace_ets_test_text"]),
        ]
        pr_caches = {
            100: _make_pr_cache(100, [
                "foundation/arkui/ace_engine/frameworks/core/components_ng/button/button_model.cpp",
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert must_run == [], f"PR {pr['pr_number']} has non-empty reviewer_decision.must_run={must_run}, should be []"
    ...
    def test_empty_reviewer_decision_must_not_run(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert must_not_run == [], f"PR {pr['pr_number']} has non-empty reviewer_decision.must_not_run={must_not_run}, should be []"
    ...
    def test_empty_reviewer_decision_expected_policy(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert expected_policy == "", f"PR {pr['pr_number']} has non-empty reviewer_decision.expected_policy='{expected_policy}', should be ''"
    ...
    def test_empty_reviewer_decision_notes(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert notes == "", f"PR {pr['pr_number']} has non-empty reviewer_decision.notes='{notes}', should be ''"
...
class TestSuggestionsPopulated:
    ...
    def test_suggested_must_run_populated_for_component_pr(self):
        ...
        suggested_must_run = pr.get("selector_suggestions", {}).get("suggested_must_run", [])
        assert len(suggested_must_run) > 0, f"PR 100 should have suggested_must_run populated, got: {suggested_must_run}"
    ...
    def test_suggested_must_not_run_populated_when_unrelated_targets(self):
        ...
        batch_results = [
            _make_batch_result(100, targets=["ace_ets_test_button", "ace_ets_test_slider", "ace_ets_test_dialog"]),
        ]
        pr_caches = {
            100: _make_pr_cache(100, [
                "foundation/arkui/ace_engine/frameworks/core/components_ng/button/button_model.cpp",
        ...
        pr = res["output"]["golden_prs"][0]
        suggested_must_not_run = pr.get("selector_suggestions", {}).get("suggested_must_not_run", [])
        # Should have at least slider and dialog as must_not_run
        assert len(suggested_must_not_run) > 0, f"PR 100 should have suggested_must_not_run populated, got: {suggested_must_not_run}"
    ...
    def test_suggested_expected_selection_populated(self):
        ...
        pr = res["output"]["golden_prs"][0]
        suggested_selection = pr.get("selector_suggestions", {}).get("suggested_expected_selection", "")
        assert suggested_selection != "", f"PR 100 should have suggested_expected_selection populated, got: '{suggested_selection}'"
    ...
    def test_suggested_policy_populated(self):
        ...
        pr = res["output"]["golden_prs"][0]
        suggested_policy = pr.get("selector_suggestions", {}).get("suggested_policy", "")
        assert suggested_policy != "", f"PR 100 should have suggested_policy populated, got: '{suggested_policy}'"
    ...
    def test_suggested_notes_populated(self):
        ...
        assert suggested_notes != "", f"PR 100 should have suggested_notes populated, got: '{suggested_notes}'"
    ...
    def test_consumer_projects_in_suggestions(self):
        ...
        pr = res["output"]["golden_prs"][0]
        consumer_projects = pr.get("selector_suggestions", {}).get("consumer_projects", [])
        assert len(consumer_projects) == 2, f"PR 100 should have 2 consumer_projects, got: {consumer_projects}"
...
class TestEmptyExpectedSelection:
    ...
    def test_expected_selection_empty(self):
        ...
        assert expected_selection == "", f"PR 100 expected_selection should be empty, got: '{expected_selection}'"
...
class TestMultiplePRs:
    ...
    def test_all_prs_candidate_status(self):
        ...
        pr_caches = {
            ...
            101: _make_pr_cache(101, [
                "foundation/arkui/ace_engine/frameworks/core/components_ng/text/text_model.cpp",
            ]),
            102: _make_pr_cache(102, [
        ...
        for pr in res["output"]["golden_prs"]:
            status = pr.get("annotation_status")
            assert status == "candidate", f"PR {pr['pr_number']} has status='{status}', expected 'candidate'"
            source = pr.get("label_source")
            assert source == "helper_script", f"PR {pr['pr_number']} has label_source='{source}', expected 'helper_script'"
    ...
    def test_all_prs_empty_reviewer_decision(self):
        ...
        for pr in res["output"]["golden_prs"]:
            ...
            assert reviewer_decision.get("must_run") == [], f"PR {pr['pr_number']} has non-empty must_run"
            assert reviewer_decision.get("must_not_run") == [], f"PR {pr['pr_number']} has non-empty must_not_run"
            assert reviewer_decision.get("expected_policy") == "", f"PR {pr['pr_number']} has non-empty expected_policy"
            assert reviewer_decision.get("notes") == "", f"PR {pr['pr_number']} has non-empty notes"
