...
def test_consumers_for_member_name_uses_parent_filter():
    ...
    idx = InvertedIndex(
        by_api={
            "api:v1:arkui.static:attribute:button#ButtonAttribute%23backgroundColor": [
                ConsumerEntry(
                    "arkui/button_test", "test.ets", 10, "chained_modifier", "strong"
                ),
            ],
            "api:v1:arkui.static:attribute:text#TextAttribute%23backgroundColor": [
                ConsumerEntry(
                    "arkui/text_test", "test.ets", 10, "chained_modifier", "strong"
                ),
            ],
        }
    )
    ...
    button_consumers = idx.consumers_for_member_name(
        "backgroundColor", parent_filter="ButtonAttribute"
    )
    ...
    text_consumers = idx.consumers_for_member_name(
        "backgroundColor", parent_filter="TextAttribute"
    )
...
def test_provenance_set_correctly_per_lookup_level():
    ...
    # Exact canonical hit
    exact = idx.consumers_for_canonical(
        "api:v1:arkui.static:attribute:button#ButtonAttribute%23role"
    )
    assert len(exact) == 1
