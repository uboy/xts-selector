...
class TestRankTargets:
    ...
    def test_single_must_run(self):
        entries = [_entry(canonical_apis=("Button/role",), reasons=({"confidence": "strong"},))]
    ...
    def test_multiple_targets_different_buckets(self):
        entries = [
            _entry(consumer_projects=("proj_must",), canonical_apis=("Api",), reasons=({"confidence": "strong"},)),
            _entry(consumer_projects=("proj_rec",), reasons=({"confidence": "strong"},)),
    ...
    def test_must_run_no_cap(self):
        entries = [
            _entry(consumer_projects=(f"proj_{i}",), canonical_apis=(f"Api{i}",), reasons=({"confidence": "strong"},))
            for i in range(50)
        ]
        result = rank_targets(entries)
    ...
    def test_duplicate_project_takes_best_bucket(self):
        entries = [
            ...
            _entry(consumer_projects=("proj_a",), canonical_apis=("Api",), reasons=({"confidence": "strong"},)),
        ]
        result = rank_targets(entries)
        assert len(result.must_run) == 1
        assert result.must_run[0].project_id == "proj_a"
    ...
    def test_scoring_sorts_descending(self):
        entries = [
            ...
            _entry(consumer_projects=("proj_2",), canonical_apis=("Api1", "Api2"), reasons=({"confidence": "strong"},)),
        ]
        result = rank_targets(entries)
    ...
    def test_all_targets_property(self):
        entries = [
            _entry(consumer_projects=("must_p",), canonical_apis=("A",), reasons=({"confidence": "strong"},)),
