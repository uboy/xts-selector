...
from .constants import CODEHUB_SECTION_NAMES
...
def _load_ini_git_host_section(
    parser: ConfigParser, section: str
    ...
    url = next(
        (
            parser.get(section, option, fallback=None)
...
def load_ini_git_host_config(
    path_value: str | None, repo_root: Path, host_kind: str
) -> tuple[str | None, str | None]:
...
def load_ini_gitcode_config(
    path_value: str | None, repo_root: Path
) -> tuple[str | None, str | None]:
...
def git_changed_files(repo_root: Path, diff_ref: str) -> list[Path]:
    ...
    return normalize_changed_files(
        completed.stdout.splitlines(), base_roots=[repo_root]
    )
...
def discover_owner_repo_from_git_remote(
    repo_root: Path, remote: str
) -> tuple[str, str] | None:
...
def resolve_pr_owner_repo(
    pr_ref: str, repo_root: Path, remote: str
) -> tuple[str, str] | None:
    """Resolve owner/repo from PR URL or git remote."""
    return parse_owner_repo_from_pr(pr_ref) or discover_owner_repo_from_git_remote(
        repo_root, remote
    )


def fetch_pr_changed_files(
    repo_root: Path, remote: str, base_branch: str, pr_ref: str
) -> list[Path]:
    ...
    for spec in fetch_specs:
        ...
        if completed.returncode == 0:
            diff = run_git(
                repo_root, ["diff", "--name-only", f"{base_ref}...{head_ref}"]
            )
            if diff.returncode != 0:
                raise RuntimeError(diff.stderr.strip() or "git diff failed")
            return normalize_changed_files(
                diff.stdout.splitlines(), base_roots=[repo_root]
            )
        last_error = completed.stderr.strip() or completed.stdout.strip() or last_error
    raise RuntimeError(last_error)
...
def resolve_pr_api_credentials(
    app_config: AppConfig, pr_ref: str
...
def fetch_git_host_api_json(
    api_kind: str, api_url: str, token: str, api_path: str | Iterable[str]
) -> object:
    ...
    api_paths = (
        ...
        else [str(item) for item in api_path if str(item)]
    )
...
def extract_pr_shas_from_api_response(
    api_kind: str, response: dict
    ...
    if api_kind == "codehub":
        base_sha = response.get("diff_refs", {}).get("base_sha") or response.get(
            "target_branch_sha"
        )
        head_sha = response.get("diff_refs", {}).get("head_sha") or response.get(
            "source_branch_sha"
        )
...
def fetch_pr_metadata_via_api(
    api_kind: str, api_url: str, token: str, owner: str, repo: str, pr_ref: str
) -> dict:
...
def fetch_pr_changed_files_via_api(
    ...
    changed_files, _changed_ranges, _raw_hunks = (
...
def fetch_pr_changed_files_and_ranges_via_api(
    ...
    if api_kind == "codehub":
        ...
        data = fetch_git_host_api_json(
            ...
            api_url,
            token,
            f"/api/v5/repos/{owner}/{repo}/pulls/{pr_number}/files",
        )
        if isinstance(data, dict):
