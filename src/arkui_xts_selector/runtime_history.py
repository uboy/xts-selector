...
class RuntimeHistoryIndex:
    ...
    capability_tool_samples: dict[tuple[str, str], list[float]] = field(
        default_factory=dict
    )
    family_tool_samples: dict[tuple[str, str], list[float]] = field(
        default_factory=dict
    )
...
def load_runtime_history(path: Path | None) -> dict[str, Any]:
    resolved = (
        default_runtime_history_file(None)
        if path is None
...
def build_runtime_history_index(path: Path | None) -> RuntimeHistoryIndex:
    ...
    index = RuntimeHistoryIndex(
        path=resolved, payload=payload, targets=dict(payload.get("targets", {}))
    )
    for target_key, entry in index.targets.items():
        if not isinstance(entry, dict):
            continue
        capabilities = list(
            entry.get("direct_capability_keys") or entry.get("capability_keys") or []
        )
        families = list(
            entry.get("direct_family_keys") or entry.get("family_keys") or []
        )
        tools = entry.get("tools", {})
        if not isinstance(tools, dict):
            continue
        for tool_name, tool_entry in tools.items():
            ...
            for capability in capabilities:
                index.capability_tool_samples.setdefault(
                    (str(capability), tool_name), []
                ).extend(samples)
            for family in families:
                index.family_tool_samples.setdefault(
                    (str(family), tool_name), []
                ).extend(samples)
    return index
...
def _estimate_from_samples(
    samples: list[float], source: str, confidence: str, tool: str
...
def estimate_target_runtime(
    ...
    capability_keys = list(
        target.get("direct_capability_keys") or target.get("capability_keys") or []
    )
    ...
    family_keys = list(
        target.get("direct_family_keys") or target.get("family_keys") or []
    )
    ...
    default_duration = float(
        DEFAULT_TOOL_DURATION_S.get(preferred_tool, DEFAULT_TOOL_DURATION_S["aa_test"])
    )
    return RuntimeEstimate(
...
def annotate_report_runtime_estimates(
    ...
    if coverage:
        ...
        coverage["estimated_required_duration_s"] = round(
            sum(
                float(item.get("estimated_duration_s", 0.0) or 0.0) for item in required
            ),
            3,
        )
        coverage["estimated_recommended_duration_s"] = round(
            ...
            + sum(
                float(item.get("estimated_duration_s", 0.0) or 0.0)
                for item in recommended_additional
            ),
            3,
        )
        coverage["estimated_all_duration_s"] = round(
            ...
            + sum(
                float(item.get("estimated_duration_s", 0.0) or 0.0) for item in optional
            ),
            3,
        )
            ...
            observations.append(
                {
                    ...
                    "direct_capability_keys": list(
                        target.get("direct_capability_keys", [])
                    ),
                    "tool": tool,
    ...
    *,
    ...
    with acquire_interprocess_lock(
        ...
        metadata=build_lock_metadata(
            "runtime_history", resolved.name, run_label=run_label
        ),
    ):
        payload = _load_runtime_history_unlocked(resolved)
        ...
        for observation in observations:
            ...
            entry = targets.setdefault(
                target_key,
                {
                    ...
                    "direct_family_keys": list(
                        observation.get("direct_family_keys", [])
                    ),
                    "capability_keys": list(observation.get("capability_keys", [])),
                    "direct_capability_keys": list(
                        observation.get("direct_capability_keys", [])
                    ),
                    "tools": {},
                },
            )
            entry["project"] = observation.get("project", "") or entry.get(
                "project", ""
            )
            entry["build_target"] = observation.get("build_target", "") or entry.get(
                "build_target", ""
            )
            for key in (
                "family_keys",
            ...
            tool_entry = tools.setdefault(
                tool_name, {"samples_s": [], "sample_count": 0}
            )
            ...
            existing_samples = existing_samples[
                -max(1, int(max_samples or DEFAULT_RUNTIME_HISTORY_SAMPLES)) :
            ]
            ...
            tool_entry["samples_s"] = [
                round(float(item), 3) for item in existing_samples
            ]
            tool_entry["sample_count"] = len(existing_samples)
            tool_entry["last_duration_s"] = round(float(observation["duration_s"]), 3)
            tool_entry["median_duration_s"] = round(
                float(new_median or observation["duration_s"]), 3
            )
            tool_entry["mean_duration_s"] = round(
                sum(existing_samples) / len(existing_samples), 3
            )
        ...
        tmp_handle, tmp_name = tempfile.mkstemp(
            prefix="runtime-history-", suffix=".json", dir=str(resolved.parent)
        )
        try:
