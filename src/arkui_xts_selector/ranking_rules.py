...
def load_ranking_rules_config(path: Path | None) -> RankingRulesConfig:
    ...
    return RankingRulesConfig(
        ...
        low_signal_specificity_tokens=_normalize_token_set(
            generic_tokens.get("low_signal_specificity", [])
        ),
        generic_coverage_extra_tokens=_normalize_token_set(
            generic_tokens.get("coverage_extra", [])
        ),
        coverage_family_group_overrides=family_groups,
        coverage_capability_group_overrides=capability_groups,
        scope_gain_multiplier={
            str(key): float(value)
            for key, value in dict(data.get("scope_gain_multiplier", {})).items()
        },
        bucket_gain_multiplier={
            str(key): float(value)
            for key, value in dict(data.get("bucket_gain_multiplier", {})).items()
        },
        ...
        umbrella_family_count_threshold=max(
            0, int(umbrella_penalties.get("family_count_threshold", 4) or 0)
        ),
        umbrella_family_count_penalty=float(
            umbrella_penalties.get("family_count_penalty", 0.05) or 0.0
        ),
        umbrella_family_count_penalty_cap=float(
            umbrella_penalties.get("family_count_penalty_cap", 0.25) or 0.0
        ),
        umbrella_penalty_cap=float(umbrella_penalties.get("penalty_cap", 0.75) or 0.0),
        umbrella_min_factor=float(
            umbrella_penalties.get("minimum_factor", 0.25) or 0.0
        ),
        family_quality_project_tokens=float(
            family_quality.get("project_tokens", 0.45) or 0.0
        ),
        family_quality_related_file_path=float(
            family_quality.get("related_file_path", 0.12) or 0.0
        ),
        family_quality_direct_file_path=float(
            family_quality.get("direct_file_path", 0.28) or 0.0
        ),
        family_quality_direct_reason_tokens=float(
            family_quality.get("direct_reason_tokens", 0.35) or 0.0
        ),
        family_quality_direct_single_family_bonus=float(
            family_quality.get("direct_single_family_bonus", 0.2) or 0.0
        ),
        family_quality_direct_small_family_bonus=float(
            family_quality.get("direct_small_family_bonus", 0.15) or 0.0
        ),
        family_quality_maximum=float(family_quality.get("maximum_quality", 2.4) or 0.0),
        family_gain_direct_base=float(
            family_quality.get("direct_gain_base", 1.0) or 0.0
        ),
        family_gain_related_base=float(
            family_quality.get("related_gain_base", 0.45) or 0.0
        ),
        family_gain_min_direct_quality=float(
            family_quality.get("minimum_direct_quality", 0.55) or 0.0
        ),
        family_gain_min_related_quality=float(
            family_quality.get("minimum_related_quality", 0.45) or 0.0
        ),
        representative_project_family_hit=float(
            representative_quality.get("project_family_hit", 0.15) or 0.0
        ),
        representative_file_family_hit=float(
            representative_quality.get("file_family_hit", 0.12) or 0.0
        ),
        representative_reason_family_hit=float(
            representative_quality.get("reason_family_hit", 0.16) or 0.0
        ),
        representative_direct_file_hit=float(
            representative_quality.get("direct_file_hit", 0.22) or 0.0
        ),
        representative_direct_family_bonus=float(
            representative_quality.get("direct_family_bonus", 0.3) or 0.0
        ),
        representative_single_family_bonus=float(
            representative_quality.get("single_family_bonus", 0.2) or 0.0
        ),
        representative_small_family_bonus=float(
            representative_quality.get("small_family_bonus", 0.12) or 0.0
        ),
        representative_source_token_overlap_weight=float(
            representative_quality.get("source_token_overlap_weight", 0.12) or 0.0
        ),
        representative_source_token_overlap_cap=float(
            representative_quality.get("source_token_overlap_cap", 0.6) or 0.0
        ),
        representative_extra_family_penalty=float(
            representative_quality.get("extra_family_penalty", 0.06) or 0.0
        ),
        representative_extra_family_penalty_cap=float(
            representative_quality.get("extra_family_penalty_cap", 0.3) or 0.0
        ),
        representative_umbrella_penalty_weight=float(
            representative_quality.get("umbrella_penalty_weight", 0.75) or 0.0
        ),
        representative_direct_overlap_multiplier=float(
            representative_quality.get("direct_overlap_multiplier", 1.0) or 0.0
        ),
        representative_related_overlap_multiplier=float(
            representative_quality.get("related_overlap_multiplier", 0.68) or 0.0
        ),
        representative_minimum_quality=float(
            representative_quality.get("minimum_quality", 0.2) or 0.0
        ),
        representative_maximum_quality=float(
            representative_quality.get("maximum_quality", 3.6) or 0.0
        ),
        planner_fallback_no_family_gain=float(
            planner.get("fallback_no_family_gain", 0.1) or 0.0
        ),
...
def apply_ranking_rules_config(config: RankingRulesConfig) -> None:
    ...
    COVERAGE_CAPABILITY_GROUP_OVERRIDES = dict(
        config.coverage_capability_group_overrides
    )
