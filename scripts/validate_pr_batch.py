...
def run_selector_on_pr(
    pr_url: str, pr_number: int, use_graph_resolver: bool = False, timeout: int = 300
) -> dict:
...
def _process_pr(args: tuple) -> dict:
    ...
    # Run selector
    try:
        result = run_selector_on_pr(
            pr_url, pr_number, use_graph_resolver=use_graph, timeout=timeout
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run selector on sampled PRs in parallel"
    )
    parser.add_argument(
        "--use-graph-resolver",
        action="store_true",
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable caching of full PR results"
    )
    ...
    urls = [
        line.strip()
        for line in pr_list_path.read_text().splitlines()
    ...
    print(
        f"Running selector ({mode_label}) on {len(sample)} PRs "
        f"(timeout={per_pr_timeout}s, workers={workers}{cache_label})...",
        flush=True,
    )
    ...
    if cached_count:
        print(
            f"Cache hits: {cached_count}/{len(work_items)} PRs already cached",
            flush=True,
        )
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(
                f"  [{completed}/{len(work_items)}] PR #{pr_number}: "
    ...
    # Print summary
    summaries = [
        extract_summary(results[pr_number]) for pr_number in sorted(results.keys())
    ]
    ...
    print(
        f"Total: {len(sample)}, OK: {ok}, Errors: {errors}, "
    ...
    if ok > 0:
        ...
        # Required/optional
        req_counts = [
            s.get("required_count", 0) for s in summaries if s["status"] == "ok"
        ]
        opt_counts = [
            s.get("optional_count", 0) for s in summaries if s["status"] == "ok"
        ]
        import statistics
        ...
        graph_resolved = [
            s
            for s in summaries
            if s["status"] == "ok" and s.get("graph_files_resolved", 0) > 0
        ]
        if graph_resolved:
            print("\nGraph resolver:")
...
def _save_results(
    results: dict[int, dict], sample: list[str], results_path: Path, summary_path: Path
) -> None:
    """Save results and summaries to disk."""
    ordered_results = [results[pr_number] for pr_number in sorted(results.keys())]
